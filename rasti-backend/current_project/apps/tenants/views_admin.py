...
def admin_customer_detail(request: HttpRequest, customer_id: int, **kwargs) -> HttpResponse:
    """Customer detail with their orders and invoices."""
    company = request.company
    customer = Customer.objects.filter(id=customer_id, company=company).first()
    if not customer:
        raise Http404("مشتری یافت نشد.")

    orders = Order.objects.filter(company=company, customer=customer).order_by("-created_at")[:20]
    invoices = Invoice.objects.filter(company=company, customer=customer).order_by("-created_at")[:20]

    return render(request, "tenants/admin_customer_detail.html", {
        "company": company, "customer": customer, "orders": orders, "invoices": invoices,
    })
    ...
    if request.method == "POST":
        try:
            ...
            service_date = parse_jalali_date(
                request.POST.get("service_date_jalali", ""), required=True,
            )
...
def admin_base_category_create(request: HttpRequest, **kwargs) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        if not title:
            ...
            return redirect(f"/{company.code}/admin/base-data/categories/{category.id}/edit/")
...
def admin_operator_list(request: HttpRequest, **kwargs) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        if action == "create_operator":
            ...
            password = request.POST.get("password") or "password123"
            is_active = request.POST.get("is_active") == "on"

            if not login_id:
                error = "شناسه ورود اپراتور الزامی است."
            else:
                login_field = get_login_field_name(User)
                if User.objects.filter(**{login_field: login_id}).exists():
                    error = "این شناسه ورود قبلاً ثبت شده است."
                else:
                    operator = User()
                    set_if_field(operator, login_field, login_id)
                    set_if_field(operator, "phone", login_id)
                    set_if_field(operator, "username", login_id)
...
def admin_base_item_create(request: HttpRequest, **kwargs) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        if category is None:
            ...
            return redirect(f"/{company.code}/admin/base-data/items/{item.id}/edit/")
...
def _rasti46_get_status_value(obj, candidates):
    for candidate in candidates:
        try:
            ...
            if raw in (None, ""):
                continue
            display_method = getattr(obj, f"get_{candidate}_display", None)
...
def admin_orders(request, **kwargs):
    ...
    if not selected_keys:
        selected_keys = [col["key"] for col in columns]
